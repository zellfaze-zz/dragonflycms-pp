...
function userinfo($username) {
	global $db, $prefix, $user_prefix, $currentlang, $pagetitle, $MAIN_CFG, $CPG_SESS, $CLASS, $BASEHREF;
...
//OpenTable();
  ...
  $myusername = $userinfo['username'];//My username

  //If not anonymous
  if (is_user()) {
    $mypost = (strtolower($poster_name) == strtolower($myusername))? ' my_post' : '';
...
/* online mod */
  ...
  <tr>
	<td class="row1" height="6" valign="top" align="center">'.$avatar.'<br>';
		/* online mod */
  //If poster is online
  if($poster_online) {
    $online_class = ' online';
    ...
    echo '<div style="color:red">'._PTY_ONLINE.'</div>';
  } else {
    echo '<i>'._PTY_OFFLINE.'</i>';
	...
	<td class="row1" rowspan="3" valign="top"><table width="100%" border="0" cellspacing="1" cellpadding="3">
		<tr>
		  ...
		  <td><b><span class="gen"><a href='.$userinfo['user_website'].'>'.$userinfo['user_website'].'</a></span></b></td>
		</tr><tr>
	...
	$rightblocksdir = dir('modules/Personal/blocks/right/');
	while ($rightfunc=$rightblocksdir->read()) {
		if (substr($rightfunc, -3) == 'php') {
			$rightblockslist[] = $rightfunc;
		}
	}
	closedir($rightblocksdir->handle);
	sort($rightblockslist);
	for ($i=0; $i < sizeof($rightblockslist); $i++) {
		require_once('modules/Personal/blocks/right/'.$rightblockslist[$i]);
	}
	echo '</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	// Left blocks
		//$user_gallery = 10000+$userinfo['user_id'];
	$leftblocksdir = dir('modules/Personal/blocks/left/');
	while ($leftfunc=$leftblocksdir->read()) {
		if (substr($leftfunc, -3) == 'php') {
			$leftblockslist[] = $leftfunc;
		}
	}
	closedir($leftblocksdir->handle);
	sort($leftblockslist);
	for ($i=0; $i < sizeof($leftblockslist); $i++) {
		require_once('modules/Personal/blocks/left/'.$leftblockslist[$i]);
	}
//CloseTable();
	if ($owninfo || can_admin('members')) {
		...
		echo '</div>';
		CloseTable();
		echo '</tr><tr></td><td colspan="2">';
		require_once('modules/Personal/centerblocks.php');
		centerblocks($username);
		//echo '</span>';
		if ($owninfo) {
...
function useLEO(){
    global $MAIN_CFG;
    return $MAIN_CFG['global']['GoogleTap'];
}